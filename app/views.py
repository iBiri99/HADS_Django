...
def index(request):  
    listaPreguntas = Question.objects.order_by('-pub_date')
    temas = {q.question_tema for q in listaPreguntas}
    template = loader.get_template('polls/index.html')
    if request.method == "POST":
        temasPOST = request.POST['subject']
        listaFiltrada = listaPreguntas.filter(question_tema = temasPOST)
    else:
        temasPOST = ""
        listaFiltrada = listaPreguntas
    context = {
                'title':'Lista de preguntas de la encuesta',
                'latest_question_list': listaPreguntas,
                'subject': temasPOST,
                'filtered_q_list':listaFiltrada,
                'subjects':temas,
              }
